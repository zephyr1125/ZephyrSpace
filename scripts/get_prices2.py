"""Get prices using close/date fields"""
...
for code, name in companies:
    ...
    if r.get('data'):
        ...
        if closes:
            print(f"{name}: 最新收盘={last_close} 日期={last_date} 近期高={max(closes):.2f} 低={min(closes):.2f}")
        else:
            print(f"{name}: {json.dumps(last, ensure_ascii=False)[:300]}")
    else:
