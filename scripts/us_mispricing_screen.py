"""美股质量错杀筛选 —— Tiger API 粗筛 + yfinance 补字段 + 量化排序.

Usage:
    python scripts/us_mispricing_screen.py                    # 用今天的日期
    python scripts/us_mispricing_screen.py --date 2026-07-23  # 指定日期
    python scripts/us_mispricing_screen.py --dry-run          # 只跑 Tiger 粗筛，不补 yfinance

Output:
    data/screens/美股质量错杀初筛_YYYY-MM-DD.csv   # Tiger 粗筛结果
    data/screens/美股质量错杀二筛排序_YYYY-MM-DD.csv # 量化排序结果
"""
...
# ── 粗筛参数 ─────────────────────────────────────────────
MIN_PRICE = 5                # 最低股价 $
MIN_MCAP_B = 2               # 最低流通市值 B$
PE_MIN, PE_MAX = 8, 30       # PE TTM 范围
...
def tiger_screen(market="US"):
    """Tiger 选股器粗筛：价格 + 市值 + PE + 跌幅，返回候选清单。
    ...
    while True:
        pages += 1
        result = client.market_scanner(market=market_enum, filters=filters, cursor_id=cursor, page_size=200)
        for item in result.items:
            fd = item.field_data
            candidates.append({
                "代码": item.symbol,
                "最新价": number(fd.get(StockField.CurPrice)),
                "流通市值_亿美元": round(number(fd.get(StockField.FloatMarketVal)) / 100_000_000, 2),
    ...
    print(f"[Tiger 粗筛] {market} 市场 → {len(candidates)} 只候选 "
          f"(PE {PE_MIN}-{PE_MAX}, 市值≥${MIN_MCAP_B}B, 价格≥${MIN_PRICE})")
    return candidates
...
def _fetch_one_yahoo(symbol):
    """用 yfinance 拉取一家公司的财务报表 + 52W高点 + 行业，计算质量指标。"""
...
def yahoo_supplement(candidates, max_workers=5):
    ...
    symbols = [c["代码"] for c in candidates]
    results = {}
    done = 0
...
def score_row(row, extra):
    ...
    valuation = 0.55 * (100 - abs((pe or 18) - 18) / 12 * 100) + 0.45 * linear(drawdown, 15, 40)
...
def _load_watchlist_symbols():
    """从 watchlist JSON 读取已在关注列表中的美股代码（去后缀）。"""
    import json
    symbols = set()
...
def main():
    parser = ArgumentParser(description="美股质量错杀筛选")
    ...
    parser.add_argument("--market", default="US", help="市场 (US/HK)")
    parser.add_argument("--skip-watchlist", action="store_true", help="排除已在 watchlist 中的标的")
    args = parser.parse_args()

    # 加载 watchlist 已有标的
    watched = _load_watchlist_symbols() if args.skip_watchlist else set()
    if watched:
        print(f"[Watchlist] 已加载 {len(watched)} 只在关注标的，将自动排除")
    ...
    # ── Step 1: Tiger 粗筛 ──
    print("=" * 60)
    print(f" 美股质量错杀筛选 — {screen_date}")
    print("=" * 60)
    candidates = tiger_screen(args.market)
    if not candidates:
    ...
    # ── 合并初筛 CSV ──
    step1_path = out_dir / f"美股质量错杀初筛_{screen_date}.csv"
    fieldnames = [
        "代码", "行业", "最新价", "流通市值_亿美元", "PE_TTM",
        "距52周高点跌幅_%", "52周高点",
    ...
    with open(step1_path, "w", encoding="utf-8-sig", newline="") as f:
        ...
        for c in candidates:
            ...
            c["行业"] = extra.get("行业", "")
            c["数据日期"] = screen_date
            c["待核验项"] = "毛利率≥25%；自由现金流最近一年为正；行业归类与普通股身份；利润质量及每股增长"
            # 合并 yfinance 字段
    ...
    scored = []
    for c in candidates:
        total, tier, penalty, flags, detail = score_row(c, supplements.get(c["代码"], {}))
        scored.append({
            "二筛排名": 0,
            "代码": c["代码"],
            "行业": c.get("行业", ""),
    ...
    step2_path = out_dir / f"美股质量错杀二筛排序_{screen_date}.csv"
