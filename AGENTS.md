...
> **流程定位**：深度分析 + PreBuy + Watchlist + 审核 + 修复 + Commit，是最完整的个股研究流程。适用于打算重仓建仓前的全面尽调，或初次覆盖某公司时建立完整知识页。

### 流程顺序

1. 深度分析（deep-company-review）
2. PreBuy 公司页（stock-prebuy-review / us-stock-prebuy / hk-prebuy）引用深度分析护城河/财务/治理评分
3. Watchlist 写入（主 Agent 统一执行，子 Agent 禁止直接写文件）
4. 双 agent 审核（公司页 + 深度分析报告，并行）
5. 事实核查与修复
6. Git Commit

> ⚠️ 深度分析在前是关键：PreBuy 中的护城河/财务质量/治理章节可直接引用深度评分，避免重复分析，也保证深度分析的更高精度不被 PreBuy 简化版覆盖。
...
- 新公司：深度总分 ≥85→Core；70-84→Growth；55-69→Radar；<55→不建议入
...
agent-review-company：审核公司页（格式 + 数据一致性）
