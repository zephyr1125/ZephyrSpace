...
用户明确要求压缩版时，可以突破篇幅下限，但必须在文档首部标注“压缩版，未完成完整三件套”。
...
> ✅ **自动入库授权**：只要完整 PreBuy/三件套复核结论满足 `S_STRATEGIC`、`A_CORE` 或 `B_GROWTH` 任一档位的全部硬门槛，且未触发重大红线，主 Agent 应自动写入对应 Watchlist 文件，无需逐家公司再次确认。
>
> **正确流程**：
