...
description: >
  ...
  v1.1 (2026-07-19): 新增 §1.5.7–1.5.12——六层验证框架、评分区间制、隐藏款不对称禁令、
  精确数字方法说明、术语精确性规则、三种组织能力拆分。信号分类新增"运营动作落地"独立行。
---
...
### 1.5.7 运营动作 ≠ 消费者热度 ≠ 商业转化（三层信号必须分层讨论）⚠️ 2026-07-19 新增

联名官宣、展览开幕、新品发售等属于**供给侧运营动作**（公司资源投入与商业合作能力），不等同于**消费者热度**（社交传播/排队/售罄/复购）或**商业转化**（GMV/补货速度/长期复购）。必须使用六层验证框架逐层标注当前所处阶段：

| 层级 | 名称 | 含义 | 能推出的结论 |
|---|---|---|---|
| L1 | 官宣 | 公司发布合作/新品/展览消息 | 公司有商业合作资源，≠ 消费者在乎 |
| L2 | 活动落地 | 联名开售/展览开幕/新品上架 | 公司有执行能力，≠ 活动成功 |
| L3 | 流量验证 | 社媒传播量/排队/预约满/搜索上升 | 出现消费者关注信号 |
| L4 | 销售验证 | 售罄/二手溢价+成交量/补货后仍热 | 消费者愿意付费 |
| L5 | 财务验证 | 财报分IP收入/利润率/复购率 | 对股东有商业价值 |
| L6 | IP 长周期复购验证 | 跨系列持续购买/脱离联名后仍有需求 | IP 具有独立生命力 |

**规则**：
- 每一层的结论只能推到该层及以下已获验证的层级，不得跨层推断。
- 当证据主要来自 L1–L2 时，结论必须使用"运营动作落地""运营资源扩散"等供给侧术语，不得使用"消费者热度上升""IP 共热"等需求侧术语。
- 联名、展览、官宣等事件可能高度相关于公司主动排期（同阶段集中投放资源），**不得将多个 L1–L2 事件线性累加为"多 IP 需求同步上升"**。

### 1.5.8 评分区间制（数据不足时禁用单点分）⚠️ 2026-07-19 新增

当关键数据（官方销售/成交量/社媒定量）缺失时，IP 健康度评分**必须使用区间**（如 ~58–62），不得输出单点分（如 62）。规则：

- ❌ 禁止：`MOLLY 62、DIMOO 61、CRYBABY 58`——这些差值（62 vs 58 = 4 分）在无精确数据支撑时没有统计意义。
- ✅ 改为：`MOLLY ~58–62、DIMOO ~58–61、CRYBABY ~55–58`，并标注「区间，置信度低」。
- 环比变化也改用定性表述：`小幅改善 / 持平 / 小幅恶化` 替代 `+6 / -2`。
- 仅在确有成交量/挂单/社媒精确数据时（如直接 API 调用），可使用单点分。

### 1.5.9 隐藏款溢价不得不对称使用 ⚠️ 2026-07-19 新增

隐藏款的高溢价主要反映**稀缺概率**（1/72 或更低的中奖率），不能直接等同于广泛消费需求。规则：

- 将 Labubu 高溢价视为"投机"、同时将 Crybaby/其他 IP 隐藏款高溢价视为"健康热度"——**属于指标不对称使用，禁止**。
- 隐藏款溢价能推出「IP 健康」的条件（必须同时满足至少两项）：① 成交量显著高于同类隐藏款历史均值；② 常规款同步售罄或溢价；③ 搜索热度扩散至非核心收藏圈；④ 补货后溢价仍维持。
- 不满足上述条件时，隐藏款溢价只能标注为「反映稀缺概率，≠广泛消费需求」。

### 1.5.10 精确数字必须附带数据方法说明 ⚠️ 2026-07-19 新增

报告中引用的任何精确数字（如"77.5 元""-51%"），必须附带或脚注说明：
- 数据来源（平台/API/新闻聚合）
- 是挂单价还是成交价
- 样本量和时间范围
- 是否排除瑕疵/拆卡/无盒
...
若以上信息不完整，该数字须降级为区间或定性表述，并标注「不可复核的高精度数字，仅供方向参考」。

### 1.5.11 术语精确性规则 ⚠️ 2026-07-19 新增

| 禁止用语 | 正确用语 | 原因 |
|---|---|---|
| 「全球第一」「全球知名度第一」 | 「泡泡玛特内部第一大 IP」 | 年报只能证明内部排名，不能证明全球所有潮玩/玩具 IP 中的排名 |
| 「Q2 财报」「Q2 官方销售数据」 | 「2026 年中期业绩 / H1 财报」 | 港股公司披露的是中期业绩，不是独立 Q2 财报 |
| 「产业资本」 | 「重要长期投资者」 | 段永平是财务投资者/价值投资者，非产业上下游方 |
| PE 数字（无口径说明） | 「PE 约 14.8 倍（可能来自 2026E 预测利润或经调整净利润，口径待确认）」 | PE 因分子（静态/动态/经调整）差异可达 2–3 倍 |
| 「黄牛全面离场」 | 「短线套利活动明显退潮」「套利空间压缩」 | 二手价跌只能证明套利空间缩小，不能证明所有黄牛离场 |
| 「持仓市值 XXX 亿」 | 「按 X 月 X 日股价估算，持仓市值约 XXX 亿」 | 动态市值不是披露事实 |
| 「多 IP 共热」「多 IP 接力兑现」 | 「多 IP 运营动作集中落地」「接力候选信号增强」 | 热度=需求侧验证，运营动作=供给侧，不可混用 |

### 1.5.12 泡泡玛特需验证的三种组织能力 ⚠️ 2026-07-19 新增

分析泡泡玛特是否具备持续制造全球 IP 的能力时，必须拆分三种不同能力，不可用一项替代另一项：

| 能力 | 含义 | 验证标的 |
|---|---|---|
| ① 发现和签约艺术家/IP 的能力 | 从零孵化或发掘新 IP | Twinkle Twinkle、HACIPUPU 等新 IP |
| ② 把中型 IP 做成全球化 IP 的能力 | 运营放大已有 IP | Molly 20 周年、Skullpanda 跨界 |
| ③ 在爆款退潮后维持长期收入的能力 | IP 生命周期管理 | 所有成熟 IP 待验证 |

- Molly/Skullpanda 联名潮主要验证第 ② 项，DIMOO×皮克斯验证第 ② 项的外部授权维度；
- **不能用成熟 IP 的联名活动完全替代对新 IP 原创孵化的验证。**
- 每周报告需在最终判断中显式标注本期主要验证了哪项能力。
...
- 非 Labubu IP 是否有接力迹象？**注意：区分"运营动作落地"（L1–L2）与"消费者需求验证"（L3–L4），措辞精确（§1.5.7）**
...
> ⚠️ **指标命名规则**：使用"非 Labubu IP 运营事件密度"（非"多 IP 催化密度"）、"多 IP 接力候选信号"（非"多 IP 共热"或"多 IP 接力兑现"）。详见 §1.5.11。
...
| 官方销售 | （填写，通常最缺） | 终端真实需求 |
| 二手投机 | （填写） | 投机热度，≠ 需求 |
| 运营动作落地 | （填写——联名/展览/官宣/新品发售等 L1–L2 事件） | 公司资源投入与商业合作能力，≠ 消费者热度，≠ 需求验证 |
| 消费者热度 | （填写——社媒传播/排队/售罄/预约满/搜索上升等 L3–L4 可观测信号） | 产品吸引力与心智 |
| 资本市场事件 | （填写） | 市场预期，独立成章 |

> 运营动作落地（L1–L2）与消费者热度（L3–L4）不得合并。详见 §1.5.7 六层验证框架。
...
| 判断 | 验证状态 |
|---|---|
| Labubu 投机退潮 | 较强确认 / 初步信号 / 未确认 |
| Labubu 消费需求退潮 | 未确认 |
| 泡泡玛特非 Labubu 运营力度增强 | 较强确认 / 初步信号 / 未确认 |
| 多个非 Labubu IP 消费热度同步上升 | 较强确认 / 初步信号 / 未确认 |
| 多 IP 已成功接棒 Labubu | 证据不足 / 初步信号 |
| 公司具备全球 IP 运营能力 | 进一步确认 / 持平 / 减弱 |
| 公司具备持续制造新全球 IP 的能力 | 仍待验证 / 进一步确认 |

### 三种组织能力验证状态（§1.5.12）

| 能力 | 本期验证标的 | 验证状态 |
|---|---|---|
| ① 发现和签约艺术家/IP 的能力 | （填写） | 待验证 / 进一步确认 |
| ② 把中型 IP 做成全球化 IP 的能力 | （填写） | 待验证 / 进一步确认 |
| ③ 爆款退潮后维持长期收入的能力 | （填写） | 待验证 |

对"泡泡玛特是否具备持续制造全球 IP 的组织能力"的验证程度：

【增强 / 持平 / 减弱 / 暂无法判断】

> ⚠️ 措辞精确：区分"全球 IP **运营/联名**能力"（如 DIMOO×皮克斯可验证）与"全球**原创** IP 制造能力"（不能直接由联名证明）。区分"接力候选信号增强"（运营动作落地）与"接力成功"（销售/财务验证）。不得偷换验证目标。
